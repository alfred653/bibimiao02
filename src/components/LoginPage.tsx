...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#121921] flex flex-col items-center justify-center p-4">
      <div style={{ '--lamp-hue': hue, '--on': isOn ? '1' : '0' } as React.CSSProperties}>
        {/* Desk lamp SVG */}
        <svg viewBox="0 0 200 260" className="w-40 h-auto mx-auto mb-8">
          {/* Base */}
          <ellipse cx="100" cy="250" rx="44" ry="10" fill="#2a3a4a" />
          {/* Stem */}
          <line x1="100" y1="250" x2="100" y2="170" stroke="#4a5a6a" strokeWidth="4" strokeLinecap="round" />
          {/* Shade */}
          <motion.path
            d="M50 170 L150 170 L130 110 L70 110 Z"
            fill={isOn ? `hsl(${hue}, 70%, 60%)` : '#3a4a5a'}
            animate={{ fill: isOn ? `hsl(${hue}, 70%, 60%)` : '#3a4a5a' }}
          />
          {/* Eyes */}
          <motion.circle cx="85" cy="145" r="5" fill="#121921"
            animate={{ r: isOn ? 5 : 3 }} />
          <motion.circle cx="115" cy="145" r="5" fill="#121921"
            animate={{ r: isOn ? 5 : 3 }} />
          {/* Pull cord */}
          <motion.line
        ...
        <AnimatePresence>
          {isOn && (
            ...
            >
              <div className="bg-[#1a2332] rounded-xl p-6 border" style={{ borderColor: 'inherit' }}>
                <SignIn />
              </div>
            </motion.div>
          )}
      ...
      <p className="text-gray-600 text-xs mt-8">下拉拉绳开灯 → 登录</p>
    </div>
  )
}
